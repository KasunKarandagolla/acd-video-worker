...
STAGE_SKILLS = {
    ...
    StageName.OPENMONTAGE_IDEA: ["openmontage-edit-planning"],
    StageName.OPENMONTAGE_SCENE_PLAN: ["openmontage-edit-planning"],
    StageName.OPENMONTAGE_ASSETS: ["openmontage-edit-planning"],
    StageName.OPENMONTAGE_EDIT: ["openmontage-edit-planning"],
    StageName.OPENMONTAGE_COMPOSE: ["openmontage-edit-planning"],
...
Use skill: football-story-strategy
""",
    StageName.FOOTAGE_DISCOVERY: """
Using footage_requirements.json, run football-source-discovery for each requirement.
...
config: dict,
        ...
        self.loopback = loopback_controller or LoopbackController(
            max_per_category=config.get("max_loopbacks", 3),
            max_total=config.get("max_total_loopbacks", 10)
        )
...
Path(config.get("openmontage_schemas_path")) if config.get("openmontage_schemas_path") else self._find_openmontage_schemas(),
    ...
    def validate_stage_inputs(self, stage: StageName) -> tuple[bool, List[str]]:
        ...
        for prev_stage in CANONICAL_STAGE_ORDER[:stage_idx]:
...
"""
    ...
    def run_pipeline(self, user_request: str, initial_context: Dict) -> StageResult:
        ...
        while True:
            ...
            # Handle failure
            if result.loopback and self.loopback.should_loopback(result.loopback.value):
                print(f"↩ Loopback to {result.loopback.value} (attempt {self.loopback.category_counts.get(result.loopback.value, 1)})")
                # Invalidate target and all downstream checkpoints
                self.checkpoint_mgr.invalidate_downstream(self.state, result.loopback)
                self.state.loopback_history.append({
                    "from_stage": next_stage.value,
                    "to_stage": result.loopback.value,
                    "reason": result.error,
                    "timestamp": datetime.utcnow().isoformat(),
                    "attempt": self.loopback.category_counts.get(result.loopback.value, 1)
                })
                self.checkpoint_mgr.save_state(self.state)
                continue
