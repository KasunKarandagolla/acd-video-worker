...
class FailureType(Enum):
    ...
    PLAYBACK_BLOCKED = "playback_blocked"  # 403 or similar
    NETWORK_FAILURE = "network_failure"
    INVALID_MEDIA = "invalid_media"
    LOW_QUALITY = "low_quality"
    DUPLICATE = "duplicate"
    UNKNOWN = "unknown"
    TRANSIENT_NETWORK = "transient_network"  # Retryable
...
class AcquisitionStatus(Enum):
    ...
    ACCEPTED = "accepted"
    REJECTED = "rejected"
    GAP = "gap"  # All candidates exhausted
...
class AcquiredSource:
    ...
    selected_timestamps: list = field(default_factory=list)  # [{"start": 0, "end": 10, "role": "hook"}]
    quality_warnings: list = field(default_factory=list)
    verification_status: str = "verified"
    acquisition_attempts: list = field(default_factory=list)  # List of AcquisitionAttempt
    file_hash: str = ""
...
class FailureClassifier:
    ...
    def classify(error: str, return_code: int = 0) -> FailureType:
        ...
        if any(kw in error_lower for kw in ["video unavailable", "removed", "deleted", "does not exist"]):
            return FailureType.REMOVED
        
        # Age restricted - check BEFORE private/login
        if any(kw in error_lower for kw in ["age restricted", "age gate", "age verification", "confirm your age"]):
            return FailureType.AGE_RESTRICTED
        
        # Private/login required
        if any(kw in error_lower for kw in ["private", "login required", "sign in", "authentication"]):
            return FailureType.PRIVATE
        
        # Geo restricted
        if any(kw in error_lower for kw in ["geo", "country", "region", "not available in your", "blocked in"]):
            return FailureType.GEO_RESTRICTED
        
        # Format unavailable
        if any(kw in error_lower for kw in ["format", "codec", "no suitable format", "requested format"]):
            return FailureType.FORMAT_UNAVAILABLE
        
        # Playback blocked (403, DRM, etc.)
        if any(kw in error_lower for kw in ["403", "forbidden", "drm", "protected", "sabr"]):
            return FailureType.PLAYBACK_BLOCKED
        
        # Network issues (transient)
        if any(kw in error_lower for kw in ["timeout", "connection", "network", "dns", "ssl", "certificate"]):
            return FailureType.TRANSIENT_NETWORK
        
        # Invalid media (corrupt, unplayable)
        if any(kw in error_lower for kw in ["corrupt", "invalid", "moov atom", "truncated", "unplayable"]):
...
class MediaValidator:
    ...
    def validate(self, file_path: str) -> tuple[bool, dict, list[str]]:
        ...
        width = 0
        height = 0
        # This would need more detailed probe data
        
        # 4. Check audio
        if self.require_audio and "audio" not in metadata:
            warnings.append("No audio track found")
            if self.require_audio:
                return False, metadata, warnings
        
        # 5. Frame sampling for visual validation
        frame_result = self._sample_frames(file_path)
        ...
        # 6. File integrity (check file size > 0)
    ...
    def _probe_file(self, file_path: str) -> dict:
        ...
        try:
            ...
            if video_stream:
                ...
                probe_data["fps"] = eval(video_stream.get("r_frame_rate", "0/1"))
    ...
    def _sample_frames(self, file_path: str, count: int = 4) -> dict:
        ...
        try:
            ...
            duration = float(json.loads(dur_result.stdout)["format"]["duration"])
...
class AcquisitionEngine:
    ...
    def acquire_for_slot(
        self,
        candidates: list,  # List of SourceCandidate
        story_slot: str,
        clip_id: str,
...
if __name__ == "__main__":
    ...
    engine = AcquisitionEngine("/tmp/test_acquisition_output")
